# =============================================================================
# Title:   modified_ces_variant() -- ces_variant() with likelihood functions
#          exposed for later CI calculation
# Purpose: Line-for-line copy of cancereffectsizeR::ces_variant() with exactly
#          one addition: the fitted per-gene likelihood function is assigned
#          into .GlobalEnv (search for "MODIFIED" below) so that downstream CI
#          code (e.g. profile-likelihood or MCMC routines in this repo) can
#          retrieve and re-evaluate it after ces_variant() returns, without
#          re-deriving it from the CESA. Everything else is unchanged from
#          upstream; diff against cancereffectsizeR::ces_variant() to confirm.
# =============================================================================
  ...
  selection_results = lapply(1:length(coverage_groups), function(i) {
    ...
    curr_results = lapply(1:num_proc_groups, function(j) {
      ...
      process_variant = function(variant_id) {
        ...
        # MODIFIED (see file header): expose this gene's fitted likelihood
        # function so CI code can re-evaluate it after ces_variant() returns.
