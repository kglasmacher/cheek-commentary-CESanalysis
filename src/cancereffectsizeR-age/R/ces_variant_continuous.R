  ...
  selection_results = lapply(1:length(coverage_groups), function(i) {
    ...
    curr_results = lapply(1:num_proc_groups, function(j) {
      ...
      process_variant = function(variant_id) {
        ...
        # Linear model, selection intensity > 0 within (minage, maxage)
         min_age <- min(as.numeric(lik_args[["sample_index"]]$group_name))
         max_age <- max(as.numeric(lik_args[["sample_index"]]$group_name))

         eval_g0 <- function(params){
           gamma0 <- params[1]
           gamma1 <- params[2]
           return(rbind(-gamma0 - min_age * gamma1 + constraint, -gamma0 - max_age * gamma1 + constraint))
         }
         ...
         fit <- nloptr(x0=par_init,
                       ...
                       ub = c(1e9,1e5),
                       eval_g_ineq = eval_g0,
                       opts = list("algorithm"="NLOPT_LN_COBYLA", #or global opt "algorithm"="NLOPT_GN_ISRES", "xtol_rel"= 1
                                   "xtol_rel"=1.0e-8,
                                   "maxeval"=10000))
      ...
      process_variant = function(variant_id) {
        ...
        if (optimizer == "bbmle"){
            ...
            eval_g0 <- function(params){
              ...
              r <- params[2]
              x0 <- params[3]
              return(rbind(-(L / (1 + exp(-r * (min_age - x0)))) + constraint,
                           -(L / (1 + exp(-r * (max_age - x0)))) + constraint))
            }
            ...
            fit <- nloptr(x0=par_init,
                          ...
                          # opts = list("algorithm"="NLOPT_GN_ISRES", #or global opt
                                      "xtol_rel"=1.0e-8,
                                      "maxeval"=10000))
...
#' 
#' @export
ces_variant_sshape <- function(cesa = NULL,
                                 variants = select_variants(cesa, min_freq = 2),
                                 samples = character(),
                                 model = "default",
                                 run_name = "auto",
  ...
  selection_results = lapply(1:length(coverage_groups), function(i) {
    ...
    curr_results = lapply(1:num_proc_groups, function(j) {
      ...
      process_variant = function(variant_id) {
        ...
        # S-shape model, set constraint: L>c, r:(-Inf,Inf)
        eval_g0 <- function(params){
          L <- exp(params[1])
          c <- exp(params[2])
          r <- params[3]
...
#' Simplified version of ces_variant to compute only basic sample counts
#' 
#' @export
ces_variant_basic_info <- function(cesa = NULL,
                                               variants = select_variants(cesa, min_freq = 2),
                                               samples = character(),
                                               model = "default",
                                               run_name = "auto",
