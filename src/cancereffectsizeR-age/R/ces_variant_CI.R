...
#' @export
ces_variant_linear_CI <- function(cesa = NULL,
  ...
  selection_results = lapply(1:length(coverage_groups), function(i) {
    ...
    curr_results = lapply(1:num_proc_groups, function(j) {
      ...
      process_variant = function(variant_id) {
        ...
        # min_age/max_age default to the full cohort's age range (used both to
        # fit the constraint and, downstream, to define the plotted x-axis).
        # Callers may override with a variant-specific range (e.g., to avoid
        # constraining/extrapolating into ages with no carriers of that variant).
        variant_min_age <- if (!is.null(min_age)) min_age else min(as.numeric(lik_args[["sample_index"]]$group_name))
        variant_max_age <- if (!is.null(max_age)) max_age else max(as.numeric(lik_args[["sample_index"]]$group_name))

        # For logistic model CI, input point estimate directly
        if (running_compound) {
          variant_id = compound_id
          selection_intensity <- lik_args$selection_results[which(lik_args$selection_results$variant_name == variant_id), c("gamma0", "gamma1")] |> unlist()
          loglikelihood <- lik_args$selection_results[which(lik_args$selection_results$variant_name == variant_id), "loglikelihood"]
        } else {
          selection_intensity <- lik_args$selection_results[which(lik_args$selection_results$variant_id == variant_id), c("gamma0", "gamma1")] |> unlist()
          loglikelihood <- lik_args$selection_results[which(lik_args$selection_results$"variant_id" == variant_id), "loglikelihood"]
        }
        ...
        # For shaded region CI using proposal density for logistic model
        if(! is.null(conf)) {
          variant_output = c(variant_output,
                             linear_si_conf_ints_adaptive_metropolis(fn,
                                                                     selection_intensity,
                                                                     loglikelihood,
                                                                     lik_args,
                                                                     n_iter = n_iter,
                                                                     sigma_gamma0 = sigma_gamma0, 
                                                                     sigma_gamma1 = sigma_gamma1,
                                                                     conf = conf, 
                                                                     df = 2,
                                                                     # --- new (adaptive) knobs ---
                                                                     adapt = TRUE,
                                                                     burn_in = burn_in,          # default: 50% of n_iter
                                                                     adapt_interval = 50,     # adapt every k iters
                                                                     eps_cov = 1e-8,
                                                                     min_age = variant_min_age,
                                                                     max_age = variant_max_age,
                                                                     seed = seed))
        }
      ...
      # Function to extract proposaldensity_results and repeat the summary fields
      subgroup_selection <- rbindlist(
        lapply(subgroup_results, function(x) {
          proposaldensity_df <- as.data.frame(x$summary$proposaldensity_results)
          colnames_df <- colnames(x$summary$proposaldensity_results)
          setnames(proposaldensity_df, colnames_df, paste0(colnames_df, "_proposaldensity"))
          summary_info <- data.frame(
            variant_id = x$summary$variant_id,
            gamma0 = x$summary$gamma0,
            gamma1 = x$summary$gamma1,
            loglikelihood = x$summary$loglikelihood,
            accept_rate = x$summary$accept_rate,
            ESS_gamma1 = x$summary$ESS_gamma1
          )
          summary_info <- summary_info[rep(1, nrow(proposaldensity_df)), ]
          cbind(summary_info, proposaldensity_df)
        }),
        fill = TRUE
      )
...
#' Same thing but for logistic
#' 
#' @export
ces_variant_logistic_CI <- function(cesa = NULL,
                                  variants = select_variants(cesa, min_freq = 2),
                                  samples = character(),
                                  model = "default",
                                  run_name = "auto",
  ...
  selection_results = lapply(1:length(coverage_groups), function(i) {
    ...
    curr_results = lapply(1:num_proc_groups, function(j) {
      ...
      process_variant = function(variant_id) {
        ...
        if (running_compound) {
          variant_id = compound_id
          selection_intensity <- lik_args$selection_results[which(lik_args$selection_results$variant_name == variant_id), c("L", "r", "x0")] |> unlist()
          loglikelihood <- lik_args$selection_results[which(lik_args$selection_results$variant_name == variant_id), "loglikelihood"]
        } else {
          selection_intensity <- lik_args$selection_results[which(lik_args$selection_results$variant_id == variant_id), c("L", "r", "x0")] |> unlist()
          loglikelihood <- lik_args$selection_results[which(lik_args$selection_results$"variant_id" == variant_id), "loglikelihood"]
        }
        ...
        if(! is.null(conf)) {
          variant_output = c(variant_output,
                             logistic_si_conf_ints_adaptive_metropolis(fn,
                                                                       selection_intensity,
                                                                       loglikelihood,
                                                                       lik_args,
                                                                       n_iter = n_iter,
                                                                       ...
                                                                       adapt = TRUE,
                                                                       burn_in = burn_in,          # default: 50% of n_iter
                                                                       adapt_interval =200,     # adapt every k iters
                                                                       eps_cov = 1e-8,
                                                                       max_attempts = max_attempts,
                                                                       first_seed = first_seed,
                                                                       shrink_lambda = shrink_lambda,
                                                                       ev_clip_min  = ev_clip_min,
                                                                       ev_clip_max  = ev_clip_max))
      ...
      subgroup_selection <- rbindlist(
        lapply(subgroup_results, function(x) {
          ...
          summary_info <- data.frame(
            ...
            loglikelihood = x$summary$loglikelihood,
            accept_rate = x$summary$accept_rate,
            ESS_r = x$summary$ESS_r,
            seed = x$summary$seed,
            attempts_used = x$summary$attempts_used
          )
          summary_info <- summary_info[rep(1, nrow(proposaldensity_df)), ]
          cbind(summary_info, proposaldensity_df)
        }),
...
#' Same thing but for sshape
#' 
#' @export
ces_variant_sshape_CI <- function(cesa = NULL,
                                  variants = select_variants(cesa, min_freq = 2),
                                  samples = character(),
                                  model = "default",
                                  run_name = "auto",
  ...
  selection_results = lapply(1:length(coverage_groups), function(i) {
    ...
    curr_results = lapply(1:num_proc_groups, function(j) {
      ...
      process_variant = function(variant_id) {
        ...
        if (running_compound) {
          variant_id = compound_id
          selection_intensity <- lik_args$selection_results[which(lik_args$selection_results$variant_name == variant_id), c("L", "c", "r", "x0")] |> unlist()
          loglikelihood <- lik_args$selection_results[which(lik_args$selection_results$variant_name == variant_id), "loglikelihood"]
        } else {
          selection_intensity <- lik_args$selection_results[which(lik_args$selection_results$variant_id == variant_id), c("L", "c", "r", "x0")] |> unlist()
          loglikelihood <- lik_args$selection_results[which(lik_args$selection_results$"variant_id" == variant_id), "loglikelihood"]
        }
        ...
        if(! is.null(conf)) {
          variant_output = c(variant_output,
                              sshape_si_conf_ints_adaptive_metropolis(fn,
                                                                      selection_intensity,
                                                                      loglikelihood,
                                                                      lik_args,
                                                                      n_iter = n_iter,
